...
def main():
    ...
    data = ['Src', 'Src_UNK', 'Target_UNK', 'Target', 'Gold_UNK', 'Gold', 'BLEU', 'BLUE_Bigram']
    csv_f.writerow(data)
	...
	if len(target_lines[index].split()) < 2:
		bleu_score = 0.0
    ...
    logging.info('Average BLEU Score: %f Bigram: %f'% (np.mean(bleu_scores), np.mean(bleu_scores_bigram)))
